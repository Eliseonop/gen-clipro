 ...
 * Devuelve pistas vacías si no hay imagen (la timeline se ve, pero sin nada).
 */
export function paperStateToTimeline(st) {
  if (!st?.hasImage) return { tracks: [], clips: [] }
  ...
  const tracks = [{
    id: PAPER_OBJECT_TRACK, kind: 'video', name: 'Objeto',
  ...
  // Los keyframes de PA ({time,x,y,scale,rotation}) se proyectan al formato del
  // editor solo para DIBUJARLOS. La fuente de verdad sigue siendo `st`; nada de
  // lo que pase aquí se guarda de vuelta salvo por `paperTimelineKfMove`.
  objectClip.keyframes = advanced
    ? {
      enabled: true,
  ...
  const clips = [objectClip]

  if (!advanced && (anim.simple.open || anim.simple.close)) {
    tracks.push({
...
export function paperTimelineSig(st) {
  ...
  return `${st.imageName}|${st.export.duration}|${anim.mode}|${kf}`
}
