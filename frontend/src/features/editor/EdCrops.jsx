...
export default function EdCrops({
  ...
  // El orden de capas se lee en la timeline (la pista de arriba va delante), así
  // que la pestaña Clip solo existe cuando hay algo que ajustar: velocidad/volumen.
  const hasProps = (isVideo || isAudio) && !isImage
  const showProps = tab === 'props' && hasProps
  ...
  return (
    <div className="ed-crops">
      <div className="ed-crops-tabs">
        <button type="button" className={`ed-tab ${!showProps ? 'on' : ''}`} onClick={() => setTab('kf')}>
          Keyframes
        </button>
        {hasProps && (
          <button type="button" className={`ed-tab ${showProps ? 'on' : ''}`} onClick={() => setTab('props')}>
            Clip
          </button>
        )}
      </div>
      ...
      {showProps ? (
        ...
        !canKeyframe(clip) && !isVideo ? (
          ...
          <>
            ...
            <div className="ed-crops-list">
              {items.map((k, i) => (
                ...
                >
                  ...
                  <span className="ed-crop-num">{i + 1}</span>
            ...
            {canKeyframe(clip) && kfBoard && onPasteKf && (
