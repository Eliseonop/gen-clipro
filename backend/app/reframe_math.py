...
def frame_at(keyframes, time: float, fallback_zoom: float = 1.0, fallback_mode: str = "smooth") -> dict:
    if not keyframes:
        return {"cx": 0.5, "cy": 0.5, "zoom": fallback_zoom, "pan_mode": fallback_mode}
    s = sorted(keyframes, key=_kf_t)
    if time <= _kf_t(s[0]):
        k = s[0]
        return {"cx": _kf_cx(k), "cy": _kf_cy(k), "zoom": _kf_zoom(k, fallback_zoom), "pan_mode": _kf_mode(k, fallback_mode)}
    last = s[-1]
    if time >= _kf_t(last):
        return {"cx": _kf_cx(last), "cy": _kf_cy(last), "zoom": _kf_zoom(last, fallback_zoom), "pan_mode": _kf_mode(last, fallback_mode)}
    for i in range(len(s) - 1):
        ...
        if ta <= time <= tb:
            ...
            if arrive == "direct":
                return {"cx": _kf_cx(a), "cy": _kf_cy(a), "zoom": za, "pan_mode": "direct"}
