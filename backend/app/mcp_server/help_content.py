...
HELP: dict[str, str] = {
    ...
    "motion": (
        ...
        "'shape' | 'html', x, y, start, end (s, end ≤ duration), opacity, "
        "scale, rotation, z_index, animation: {entrance, exit}, effect}. ORIGEN de x,y: para "
        "'text'/'shape' es el CENTRO en px; para 'html' a pantalla completa usa x=0, y=0, "
        "width=W, height=H (arriba-izquierda, llena el lienzo) — NO lo centres con x=W/2,y=H/2.\n"
        ...
        "PLANTILLAS VISUALES (antes que las de texto): stack_list (elementos que caen y se "
        "apilan), card_grid (cuadrícula), sequence_rows (filas editoriales), versus (cara a "
        "cara), before_after, flow_steps (proceso con flechas), decision_tree, concept_map, "
        "line_graph (trazo que se dibuja), timeline_track (hitos fechados), asset_showcase "
        "(PNG del material como protagonistas), annotate (flecha sobre el vídeo), assemble "
        "(piezas que convergen). Reciben contenido: items=[{label, sublabel?, image?}] con "
        "image = id de una imagen del material (se incrusta el PNG REAL; nunca dibujes un logo "
        "falso). motion_list_templates trae best_for/slots de cada una. Las de categoría "
        "'user' (u_…) son composiciones guardadas por el usuario: se rellenan con "
        "params={texts:{original:nuevo}, images:{id_original:id_nuevo}}.\n"
        "PLANTILLAS de texto: pro_title (antetítulo+título+subrayado), "
