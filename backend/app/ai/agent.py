...
def _system_prompt(context_summary: str | None, context: dict | None) -> str:
    ...
    if ctx.get("surface") == "motion_studio":
        ...
        lines.append(
            ...
            "(p.ej. 1920x1080 para 16:9). En cambio, las capas type:'html' (bloque HTML+GSAP) "
            "a pantalla completa usan x=0, y=0, width=W, height=H (origen ARRIBA-IZQUIERDA; "
            "llenan el lienzo) — NO las centres con x=W/2,y=H/2. Cada capa tiene start/end (segundos) y "
